...
export default function Chat() {
  ...
  return (
    <>
      <div className="bg-white top-14 right-auto md:right-20 p-4 h-auto w-72 rounded-lg transition-all duration-300 drop-shadow-md fixed z-50 dark:bg-secondary-dark-bg dark:text-white">
        ...
        <div className=" text-black">
          <div className="items-center mb-4 justify-start">
            {todo.map((item) => (
              <div
                className="flex border-b-2 dark:border-gray-600 my-4 justify-between transition-all duration-300 dark:text-white"
                key={item.id}
              >
                {" "}
        ...
        <div className="flex bottom-0 mt-4 bg-white text-gray-700 dark:bg-secondary-dark-bg dark:text-white">
          <input
            value={todo_item}
          ...
          <button
            type="submit"
            onClick={() => {
              addTodo({ id: Date.now(), value: todo_item });
              setTodo_item("");
            }}
            style={{ backgroundColor: currentColor }}
